...
public class MineSweeping {
    ...
    public BufferedImage toImg() throws IOException {
        BufferedImage bf = new BufferedImage(16 * board[0].length, 16 * board.length, BufferedImage.TYPE_INT_BGR);
        Graphics graphics = bf.getGraphics();
        for (int i = 0; i < board[0].length; i++) {
            for (int j = 0; j < board.length; j++) {
                if (board[i][j].isOpen) {
                    graphics.drawImage(ImageIO.read(new File("runFile/minesweeping/" + board[i][j].getNumber() + ".bmp")), 16 * i, 16 * j,null);
                } else if (board[i][j].isFlag()){
                    graphics.drawImage(ImageIO.read(new File("runFile/minesweeping/13.bmp")), 16 * i, 16 * j,null);
                } else {
                    graphics.drawImage(ImageIO.read(new File("runFile/minesweeping/9.bmp")), 16 * i, 16 * j,null);
                }
            }
        }
        return bf;
        ...
        for (int i = 0; i < board[0].length; i++) {
            for (int j = 0; j < board.length; j++) {
                if (board[i][j].number != 9) {
                    if (board[i][j].isFlag()){
                        graphics.drawImage(ImageIO.read(new File("runFile/minesweeping/11.bmp")), 16 * i, 16 * j,null);
                    } else {
                        graphics.drawImage(ImageIO.read(new File("runFile/minesweeping/" + board[i][j].getNumber() + ".bmp")), 16 * i, 16 * j,null);
                    }
                } else {
                    if (board[i][j].isFlag()){
                        graphics.drawImage(ImageIO.read(new File("runFile/minesweeping/10.bmp")), 16 * i, 16 * j,null);
                    } else {
                        graphics.drawImage(ImageIO.read(new File("runFile/minesweeping/12.bmp")), 16 * i, 16 * j,null);
                    }
                }
            }
        }
